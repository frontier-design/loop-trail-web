 ...
 *
 * 6 columns on desktop, 8 on tablet (≤ BREAKPOINT_TABLET),
...
export const Grid = styled.div`
  ...
  @media ${GRID.MEDIA_LARGE} {
    max-width: ${props => (props.$fullBleed ? '100%' : `min(${GRID.MAX_WIDTH_LARGE}px, 100%)`)};
    padding: ${props => (props.$fullBleed ? '0' : `0 ${GRID.PADDING_LARGE}px`)};
  }
 ...
 *   <GridCell $span={6} $subgrid>              → subgrid spanning all 6 cols
 */
 ...
 * Desktop grid has 6 columns; spans > 6 are capped at full width.
 *
 *   <GridSpan4>   → 4 columns
 *   <GridSpan6>   → 6 columns (full width on desktop)
 *   <GridSpan8>   → 6 columns (full width on desktop)
 *   <GridSpan12>  → 6 columns (full width on desktop)
 *
 * You can still pass any GridCell prop to override:
 *   <GridSpan6 $start={2} $spanMobile={2}>…</GridSpan6>
 */
